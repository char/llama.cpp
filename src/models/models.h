...
struct llama_model_aliceai_t5_moe : public llama_model_base {
    llama_model_aliceai_t5_moe(const llama_model_params & params) : llama_model_base(params) {}
    void load_arch_hparams(llama_model_loader & ml) override;
    void load_arch_tensors(llama_model_loader & ml) override;

    uint32_t n_head_kv_enc = 0;
    std::vector<llama_layer> layers_enc;

    template <bool is_enc>
    struct graph : public llm_graph_context {
        graph(const llama_model_aliceai_t5_moe & model, const llm_graph_params & params);
    };
